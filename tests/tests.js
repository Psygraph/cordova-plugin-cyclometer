...
/* global Windows */
    ...
    // Checking existence of cyclometer for windows platform
    // Assumed that cyclometer always exists on other platforms. Extend
    // condition to support cyclometer check on other platforms
    isAccelExist = isWindows ? Windows.Devices.Sensors.Accelerometer.getDefault() !== null : true;
    
    describe('Cyclometer (navigator.cyclometer)', function () {
            ...
            it("cyclometer.spec.1 should exist", function () {
                    expect(navigator.cyclometer).toBeDefined();
                });
            
            describe("getCurrentAcceleration", function() {
                    it("cyclometer.spec.2 should exist", function() {
                            expect(typeof navigator.cyclometer.getCurrentAcceleration).toBeDefined();
                            expect(typeof navigator.cyclometer.getCurrentAcceleration == 'function').toBe(true);
                        });
                    
                    it("cyclometer.spec.3 success callback should be called with an Acceleration object", function(done) {
                            ...
                            navigator.cyclometer.getCurrentAcceleration(win, onError);
                        });
                    
                    it("cyclometer.spec.4 success callback Acceleration object should have (reasonable) values for x, y and z expressed in m/s^2", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
                            ...
                            navigator.cyclometer.getCurrentAcceleration(win, fail.bind(null,done));
                        });
                    
                    it("cyclometer.spec.5 success callback Acceleration object should return a recent timestamp", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
            ...
            describe("watchAcceleration", function() {
                    ...
                    afterEach(function(done) {
                            if (id) {
                                navigator.cyclometer.clearWatch(id);
                            }
                    ...
                    it("cyclometer.spec.6 should exist", function() {
                            expect(navigator.cyclometer.watchAcceleration).toBeDefined();
                            expect(typeof navigator.cyclometer.watchAcceleration == 'function').toBe(true);
                        });
                    
                    it("cyclometer.spec.7 success callback should be called with an Acceleration object", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
                            ...
                            id = navigator.cyclometer.watchAcceleration(win, fail.bind(null,done), {frequency:100});
                        });
                    
                    it("cyclometer.spec.8 success callback Acceleration object should have (reasonable) values for x, y and z expressed in m/s^2", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
                    ...
                    it("cyclometer.spec.9 success callback Acceleration object should return a recent timestamp", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
                    ...
                    it("cyclometer.spec.12 success callback should be preserved and called several times", function (done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
            ...
            describe("clearWatch", function() {
                    it("cyclometer.spec.10 should exist", function() {
                            expect(navigator.cyclometer.clearWatch).toBeDefined();
                            expect(typeof navigator.cyclometer.clearWatch == 'function').toBe(true);
                        });
                    
                    it("cyclometer.spec.11 should clear an existing watch", function(done) {
                            // skip the test if Accelerometer doesn't exist on this device
                            if (!isAccelExist) {
                                pending();
                            }
                            ...
                            id = navigator.cyclometer.watchAcceleration(function() { win(); }, fail.bind(null, done), {frequency:100});
                        });
                });
        });
};
...
exports.defineManualTests = function (contentEl, createActionButton) {
    ...
    function stopAccel() {
        ...
        if (watchAccelId) {
            navigator.cyclometer.clearWatch(watchAccelId);
            watchAccelId = null;
        }
    }
    ...
    var watchAccel = function () {
        ...
        var opt = {};
        opt.frequency = 1000;
        watchAccelId = navigator.cyclometer.watchAcceleration(success, fail, opt);
    ...
    var getAccel = function () {
        ...
        // Make call
        var opt = {};
        navigator.cyclometer.getCurrentAcceleration(success, fail, opt);
    };

    /******************************************************************************/

    var cyclometer_tests = '<div id="getAcceleration"></div>' +
    ...
    'Expected result: When pressed, will start a watch on the cyclometer and update X,Y,Z values when movement is sensed. Status will read "Running"' +
    '<p/> <div id="clearAcceleration"></div>' +
    'Expected result: Will clear the cyclometer watch, so X,Y,Z values will no longer be updated. Status will read "Stopped"';
